...
graph_gen_acctype <- function (df) {
  ...
  # Create an affiliation matrix connecting users to regions
  user_acctype_matrix <- df %>%
    select(user = Author, acctype = AccountType) %>% 
    table() %>% 
    unclass() %>% 
    Matrix()
  
  shared_acctype_matrix <- user_acctype_matrix %*% t(user_acctype_matrix)
  
  return(shared_acctype_matrix)
}
```
